...
/*=================================================================*/
	...
	private void findTypeByPage(HttpServletRequest request, HttpServletResponse response) {
	...
	private void addOrUpdate(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		ResultInfo<Note> resultInfo = noteService.addOrUpdate(typeId, title, content, userId,noteId);


		// 3、判断是否更新成功
		if (resultInfo.getCode() == 1) { // 成功则挑转到论坛
			// 重定向跳转到论坛	
			response.sendRedirect("http://localhost:8080/newbosses/typeServlet?actionName=noteCount");
		} else {
