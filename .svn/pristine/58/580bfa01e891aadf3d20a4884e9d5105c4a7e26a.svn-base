...
public class ChatDao {
	...
	@SuppressWarnings("unchecked")//大概不用了,没往前面看
	...
	public List<ChatList> findNoteListByPage(Integer index, Integer pageSize, String local) {
		//定义sql语句
		String sql = "select title,pubTime,typeName,schName,content,noteId from (select noteId,title,content,typeName,userId,pubTime from new_note n,new_type t where t.typeId=n.typeId) m left outer join"
		+"(select uName,schName,schLocal,u.userId from new_user u,new_school s where u.schId = s.schId) u "
	 ...
	 * @return
	 */
	public long searchNoteCount(String title) {
		//定义sql语句
		String sql = "select count(1) from (select title,content,typeName,userId,pubTime from new_note n,new_type t where t.typeId=n.typeId) m left outer join"
				+"(select uName,schName,schLocal,u.userId from new_user u,new_school s where u.schId = s.schId) u "
				+"on m.userId = u.userId  where title like concat('%',?,'%') ";
...
//	 * 搜索帖子内容有【分页】
	 ...
	 */
	@SuppressWarnings("unchecked")
	public List<ChatList> searchNoteListByPage(Integer index, Integer pageSize, String title) {
		// 定义sql语句
		String sql = "select title,pubTime,typeName,schName,content,'search' as sel from (select title,content,typeName,userId,pubTime from new_note n,new_type t where t.typeId=n.typeId) m left outer join"
				+"(select uName,schName,schLocal,u.userId from new_user u,new_school s where u.schId = s.schId) u "
				+"on m.userId = u.userId  where title like concat('%',?,'%') ";
		// 2、设置参数集合
		List<Object> params = new ArrayList<Object>();
